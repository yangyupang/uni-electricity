<template>
	<view>
		<scroll-view v-if="subList" scroll-x="true" class="header-nav" :scroll-into-view="`t${active}`">
		...
		methods: {
			getCategoryNav(id) {
				this.$api.getCategoryNav(id).then(res => {
					// console.log(res);
					this.subList = res.data.navData
					this.active = res.data.currentNav.id
			...
			getCategoryGoods(id) {
				this.$api.getCategoryGoods(id).then(res => {
